...
```
【摘要】MySQL 加锁处理分析 本文来自：何登成的技术博客   一、背景 MySQL/InnoDB的加锁分析，一直是一个比较困难的话题。我在工作过程中，经常会有同事咨询这方面的问题。同时，微博上也经常会收到MySQL锁相关的私信，让我帮助解决一些死锁的问题。本文，准备就MySQL/InnoDB的加锁问题，展开较为深入的分析与讨论，主要是介绍一种思路，运用此思路，拿到任何一条SQL语句，都能...
```
...
![MySQL 加锁处理分析（MVVC、快照读、当前读、GAP锁（间隙锁））——数据库锁看这一篇就足够了1](https://res-static.hc-cdn.cn/fms/img/c391f56ae077738c088a69e5d9fd3eed1603799550970)
...
![MySQL 加锁处理分析（MVVC、快照读、当前读、GAP锁（间隙锁））——数据库锁看这一篇就足够了2](https://res-static.hc-cdn.cn/fms/img/6f2ac0f5fbafbffc10a7127d9034bd8e1603799550971)
...
![MySQL 加锁处理分析（MVVC、快照读、当前读、GAP锁（间隙锁））——数据库锁看这一篇就足够了3](https://res-static.hc-cdn.cn/fms/img/332fb831dbb6de3704f16bd08b43f0951603799550971)
...
![MySQL 加锁处理分析（MVVC、快照读、当前读、GAP锁（间隙锁））——数据库锁看这一篇就足够了4](https://res-static.hc-cdn.cn/fms/img/968c1fb9430622757d85db998f55b5ee1603799550971)
...
![MySQL 加锁处理分析（MVVC、快照读、当前读、GAP锁（间隙锁））——数据库锁看这一篇就足够了5](https://res-static.hc-cdn.cn/fms/img/242c891154bb3b50a240dc3fbe0acc631603799550972)
...
![MySQL 加锁处理分析（MVVC、快照读、当前读、GAP锁（间隙锁））——数据库锁看这一篇就足够了6](https://res-static.hc-cdn.cn/fms/img/f92ddce6bdf1c0436974763e6697b26a1603799550972)
...
![MySQL 加锁处理分析（MVVC、快照读、当前读、GAP锁（间隙锁））——数据库锁看这一篇就足够了7](https://res-static.hc-cdn.cn/fms/img/b8d2e912d6f3e4fe69782f370aa3e3e71603799550972)
...
![MySQL 加锁处理分析（MVVC、快照读、当前读、GAP锁（间隙锁））——数据库锁看这一篇就足够了8](https://res-static.hc-cdn.cn/fms/img/823083f585adcb18dc002a5fc64b845c1603799550973)
...
![MySQL 加锁处理分析（MVVC、快照读、当前读、GAP锁（间隙锁））——数据库锁看这一篇就足够了9](https://res-static.hc-cdn.cn/fms/img/c0009a9fe83df8391dce944551bdf8d51603799550973)
...
![MySQL 加锁处理分析（MVVC、快照读、当前读、GAP锁（间隙锁））——数据库锁看这一篇就足够了10](https://res-static.hc-cdn.cn/fms/img/107688b9833bc04d94c9649f40ace1231603799550973)
...
![MySQL 加锁处理分析（MVVC、快照读、当前读、GAP锁（间隙锁））——数据库锁看这一篇就足够了11](https://res-static.hc-cdn.cn/fms/img/53abaeb5e5aefad98d02e0240739dcc41603799550974)

![MySQL 加锁处理分析（MVVC、快照读、当前读、GAP锁（间隙锁））——数据库锁看这一篇就足够了12](https://res-static.hc-cdn.cn/fms/img/f08513e2d2380ea4a428c40ee9c114cb1603799550974)

上面的两个死锁用例。第一个非常好理解，也是最常见的死锁，**每个事务**执行两条SQL，分别持有了一把锁，然后加另一把锁，产生死锁。
...
**转载自：https://www.cnblogs.com/tutar/p/5878651.html**
