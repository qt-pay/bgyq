...
原文：https://github.com/yacan8/blog/issues/28
...
![](https://image-1300760561.cos.ap-beijing.myqcloud.com/bgyq-blog/browser-1.png)
...
![](https://image-1300760561.cos.ap-beijing.myqcloud.com/bgyq-blog/browser-2.png)
...
![](https://image-1300760561.cos.ap-beijing.myqcloud.com/bgyq-blog/browser-3.png)
...
![](https://image-1300760561.cos.ap-beijing.myqcloud.com/bgyq-blog/browser-4.png)
...
- site-instance指的是一组connected pages from the same site，这里connected的定义是can obtain references to each other in script code怎么理解这段话呢。满足下面两中情况并且打开的新页面和旧页面属于上面定义的同一个 site，就属于同一个site-instance
...
- network thread：处理网络请求，从网上获取数据（Chrome72以后，已将network thread单独摘成network service process，当然也可以通过 chrome://flags/#network-service-in-process修改配置，将其其作为线程运行在Browser Process中，感谢 [@Popeye-Wz](https://github.com/Popeye-Wz) 的提出）；
- storage thread： 控制文件等的访问；

![](https://image-1300760561.cos.ap-beijing.myqcloud.com/bgyq-blog/browser-5.png)

#### 第一步：处理输入

当我们在浏览器的地址栏输入内容按下回车时，`UI thread`会判断输入的内容是搜索关键词（search query）还是URL，如果是搜索关键词，跳转至默认搜索引擎对应都搜索URL，如果输入的内容是URL，则开始请求URL。

![](https://image-1300760561.cos.ap-beijing.myqcloud.com/bgyq-blog/browser-6.png)
...
![](https://image-1300760561.cos.ap-beijing.myqcloud.com/bgyq-blog/browser-7.png)
...
![](https://image-1300760561.cos.ap-beijing.myqcloud.com/bgyq-blog/browser-8.png)
...
![](https://image-1300760561.cos.ap-beijing.myqcloud.com/bgyq-blog/browser-9.png)
...
![](https://image-1300760561.cos.ap-beijing.myqcloud.com/bgyq-blog/browser-10.png)

不同的线程，有着不同的工作职责。

#### 构建DOM:heavy_check_mark:
...
![](https://image-1300760561.cos.ap-beijing.myqcloud.com/bgyq-blog/browser-11.png)
...
![](https://image-1300760561.cos.ap-beijing.myqcloud.com/bgyq-blog/browser-12.png)
...
![](https://image-1300760561.cos.ap-beijing.myqcloud.com/bgyq-blog/browser-13.png)
...
![](https://image-1300760561.cos.ap-beijing.myqcloud.com/bgyq-blog/browser-14.png)

#### 合成 - Compositing: 666

文档结构、元素的样式、元素的几何关系、绘画顺序，这些信息我们都有了，这个时候如果要绘制一个页面，我们需要做的是把这些信息转化为显示器中的像素，这个转化的过程，叫做`光栅化`（rasterizing）。

那我们要绘制一个页面，最简单的做法是只光栅化视口内（viewport）的网页内容，如果用户进行了页面滚动，就移动光栅帧（rastered frame）并且光栅化更多的内容以补上页面缺失的部分，如下是个最简单的光栅化：

![](https://image-1300760561.cos.ap-beijing.myqcloud.com/bgyq-blog/browser-15.gif)
...
![](https://image-1300760561.cos.ap-beijing.myqcloud.com/bgyq-blog/browser-16.gif)
...
![](https://image-1300760561.cos.ap-beijing.myqcloud.com/bgyq-blog/browser-17.png)
...
![](https://image-1300760561.cos.ap-beijing.myqcloud.com/bgyq-blog/browser-18.png)
...
![](https://image-1300760561.cos.ap-beijing.myqcloud.com/bgyq-blog/browser-19.png)
...
![](https://image-1300760561.cos.ap-beijing.myqcloud.com/bgyq-blog/browser-20.png)

### 渲染进程中合成器线程接收事件：非快速滚动
...
即non-fast scrollable region需要浏览器等待js执行，不会使用合成新的帧。

![](https://image-1300760561.cos.ap-beijing.myqcloud.com/bgyq-blog/browser-21.png)
...
![](https://image-1300760561.cos.ap-beijing.myqcloud.com/bgyq-blog/browser-22.png)
...
![](https://image-1300760561.cos.ap-beijing.myqcloud.com/bgyq-blog/browser-23.png)
...
![](https://image-1300760561.cos.ap-beijing.myqcloud.com/bgyq-blog/browser-24.png)

出于优化的目的，浏览器会合并这些连续的事件，延迟到下一帧渲染是执行，也就是`requestAnimationFrame`之前。

![](https://image-1300760561.cos.ap-beijing.myqcloud.com/bgyq-blog/browser-25.png)
