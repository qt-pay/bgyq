...
声明了一个整数变量，类型为int，值为9（二进制写法为1001）。普通的32位计算机，用4个字节表示int变量，所以9就被保存为00000000 00000000 00000000 00001001，写成16进制就是0x00000009。
...
如下，为什么不显示`00000000000000000001001`

```go
// 25代表精度，这种方式会有小数点后无效的0的情况
fmt.Printf("%.25f\n", *fp)
// output
